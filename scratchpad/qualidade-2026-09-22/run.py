...
parser.add_argument("tool", choices=["meter"])
...
with tempfile.TemporaryDirectory(prefix="tradutor-qualidade-build-") as tmp:
    ...
    if a.args and a.args[0] in ("suite", "trials"):
        subprocess.run(["python3", str(here / (a.args[0]+".py")), str(exe), *a.args[1:]], cwd=root, check=True)
    else:
