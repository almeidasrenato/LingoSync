...
/// unidade era a palavra separada por espaço. Japonês, chinês e coreano não
...
/// reconhecedor.
public enum ReceivedTokens {

    /// Escrita que não separa palavra com espaço.
    ///
    /// Kana, kanji, hangul e a pontuação de largura inteira que vem com eles.
...
public struct ReceivedTracker {
    ...
    public mutating func feed(_ hypothesis: String) -> [String] {
        let words = ReceivedTokens.split(hypothesis)
...
public struct ReceivedAccumulator {
    ...
    public var current: String { ReceivedTokens.join(words) }
