...
enum AppleSpeechLocales {
    ...
    static func locale(for language: Language) async -> Locale? {
        let hint = language.appleLocaleHint
...
public final class AppleSpeechTranscriber: Transcriber, @unchecked Sendable {
    ...
    public func prepare(progress: @escaping @Sendable (Double, String) -> Void) async throws {
        ...
        guard await AppleSpeechLocales.isInstalled(locale) else {
    ...
    static func phrases(
        ...
        for run in result.text.runs {
            ...
            // Rede para o run que sozinho já passa do teto: a conferência de
            // cima não tem onde cortar quando o trecho ainda estava vazio.
    ...
    public static let softCeiling: Double = 5
    ...
    public static let hardCeiling: Double = 7
...
@MainActor
@Observable
public final class AppleSpeechLanguages {

    public static let shared = AppleSpeechLanguages()
    ...
    public func refresh() async {
        ...
        for language in Language.allCases {
            guard let locale = await AppleSpeechLocales.locale(for: language) else { continue }
            supported.append(language)
            if await AppleSpeechLocales.isInstalled(locale) { installed.append(language) }
        }
